...
module.exports.test = () => contract("InitialOffering", ([user1, user2, user3, user4]) => {
    ...
    it("Should claim Hegic after the offering (was provided enough)", async () => {
        ...
        try {
            ...
            await timeTravel(delta)
            await InitialOffering.sendTransaction({value:toWei(100), from:user1})
            await InitialOffering.sendTransaction({value:toWei(200), from:user2})
            await InitialOffering.sendTransaction({value:toWei(50), from:user4})
            await InitialOffering.sendTransaction({value:toWei(100), from:user2})
            await InitialOffering.sendTransaction({value:toWei(200), from:user3})
